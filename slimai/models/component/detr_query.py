...
class DETRQuery(torch.nn.Module):
  def __init__(self, 
    ...
    self.query_embed = torch.nn.Parameter(torch.randn(1, num_query, input_dim), requires_grad=True)
    return
  
  def forward(self, x):
    x = x.view(x.size(0), -1, self.input_dim)
    query_embed = self.query_embed.repeat(x.size(0), 1, 1)
    query = self.decoder(query_embed, x)
    return query
