...
def segment_foreground_mask(wsi, 
                            speed_up: float=1.0, 
                            kernel_size: int=5, 
                            iterations: int=3):
  """
  ...
  wsi = scale_image.to_batch_numpy_image(wsi)[0] # make it to [H, W, ?C] in dtype=uint8
  ...
  if isinstance(wsi, torch.Tensor):
    mask = torch.as_tensor(mask, device=wsi.device)

  return mask

def segment_background_mask(wsi, speed_up=1, kernel_size=5, iterations=3):
  return ~segment_foreground_mask(wsi, speed_up, kernel_size, iterations)
