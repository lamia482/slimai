...
class DetectionVisualizer(Visualizer):
  def _visualize(self, image, gt_instance, pred_instance, names, 
                 score_thr=0.01):
    gt_instance = gt_instance["instance"]
    image = to_image(image)
    image = put_gt_on_image(image, gt_instance, names, color=(255, 0, 0), no_text=False)
    image = put_pred_on_image(image, pred_instance, names, score_thr=score_thr, color=(0, 0, 255), no_text=False)
    return image
